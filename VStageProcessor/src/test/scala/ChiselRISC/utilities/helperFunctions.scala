...
object helperFunctions {
  def FillInstructionMemory(inputString: String, clock: Clock, interface: writeToInstMem): Unit = {
    /**
    ...
    * @param interface: Interface of the instruction memory.
    ...
    interface.ready.poke(true.B) // Set the ready signal to true
    for (i <- 0 until amountOfInstructions) { // Loop through the instructions
      ...
      interface.data.poke(inst.U(32.W))
      clock.step(1)
    }
    interface.ready.poke(false.B)
    interface.data.poke(0.U(32.W))
  }

  def FillInstructionMemoryFromFile(instructions: String, clock: Clock, interface: writeToInstMem): Unit = {
