 ...
 * write should however only be for testing purposes
 */
class InstructionMemory(depth: Int, datawidth: Int) extends Module {
  val bidwidth = log2Ceil(depth) // Calculate the number of bits needed to address the memory
  val actualDepth = math.pow(2, bidwidth).toInt // 2^bidwidth

  val io = IO(new Bundle { // Input and Output
    val writeMem = Flipped(new DecoupledIO(datawidth.U)) // Write to memory
    val rdAdd = Flipped(new DecoupledIO(datawidth.U)) // Read address
  ...
  val readAddr = WireInit(0.U(bidwidth.W))

  // Pointers to handle empty and full logic
  val readPtr = RegInit(0.U(bidwidth.W)) // Read pointer
  val writePtr = RegInit(0.U(bidwidth.W)) // Write pointer
  val count = RegInit(0.U((bidwidth + 1).W)) // Count of the number of elements in the memory
  ...
  when(io.rdAdd.valid && io.inst.ready && count =/= 0.U ){
    ...
    readAddr := io.rdAdd.bits >> 2 // Divide by 4 to get the correct read address
