...
class SingleCycleProcessorInstWrapper(
  memoryFile: String,
) extends SingleCycleProcessor() {
  val registers    = expose(registerFile.registers)
  val pc           = expose(programCounter.io.counter)
  val memWriteAddr = expose(memory.io.writeAddress)
  val memWriteData = expose(memory.io.writeData)
  val memReadAddr  = expose(memory.io.readAddress)
  val memReadData  = expose(memory.io.readData)
}

class SingleCycleProcessorInstructionSpec
  extends AnyFlatSpec
  ...
  def defaultDut(
    ...
    test(new SingleCycleProcessorInstWrapper(memoryfile.relativeTo(os.pwd).toString))
      .withAnnotations(
        Seq(
          WriteVcdAnnotation,
        ),
  ...
  behavior of "SingleCycleProcessorInstruction"
